...
const CampSite = ({backgroundImage, title, subtitle, pepoleJoined}: CampProps) => {
  return (
       ...
       lg:px-20 lg:py-10 ">
        ...
        <div className="flexCenter gap-6 ">
          <span className="flex -space-x-4 overflow-hidden">
            {PEOPLE_URL.map((url) => (
              <Image
              className="inline-block h-10 w-10 rounded-4"
                src={url}
                key={url}
                alt="person"
                width={52}
          ...
          <p className="bold-16 md:bold-20 text-white">{pepoleJoined}</p>
        </div>
      </div>
    </div>
  )
...
function Camp() {
  return (
    <section className="2xl:max-container relative flex flex-col py-10 m-auto
     lg:mb-10 lg:py-20 xl:mb-20">
        ...
        <div className="flexEnd mt-10 px-6 lg:-mt-60 lg:mr-6">
          <div className="bg-green-50 p-8 lg:max-2-[500px] xl:max-w-[734px]
          xl:rounded-5xl xl:px-16 xl:py-20 relative w-full overflow-hidden rounded-3xl">
            <h2 className="regular-24 md:regular-42 2xl:regular-64 capitalize 
            text-white">
